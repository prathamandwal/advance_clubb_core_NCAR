...
!#######################################################################
        ...
        SUBROUTINE pdf_closure_test(nlevel,hydromet_dim, p_in_pa, exner, thv_ds, wm, wp2, wp3, sigma_sqd_w, skw, rtm, rtp2, wprtp, thlm, &
        thlp2, wpthlp, rtpthlp, sclrm, wpsclrp, sclrp2, sclrprtp, sclrpthlp, wphydrometp, wp2hmp, rtphmp, thlphmp, wp4, &
...
! Number of hydrometeor species              [#]
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: rtpthlp
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: wp3
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: p_in_pa
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: wprtp
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: sigma_sqd_w
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: rtm
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: exner
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: rtp2
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: skw
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: thv_ds
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: thlm
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: wm
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: thlp2
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: wp2
            REAL(KIND=core_rknd), intent(in),dimension(nlevel) :: wpthlp
! Pressure                                   [Pa]
! Exner function                             [-]
...
! w'th_l'                                    [K(m/s)]
! r_t'th_l'                                  [K(kg/kg)]
            REAL(KIND=core_rknd), dimension(nlevel,sclr_dim), intent(in) :: sclrm
            REAL(KIND=core_rknd), dimension(nlevel,sclr_dim), intent(in) :: wpsclrp
            REAL(KIND=core_rknd), dimension(nlevel,sclr_dim), intent(in) :: sclrp2
            REAL(KIND=core_rknd), dimension(nlevel,sclr_dim), intent(in) :: sclrprtp
            REAL(KIND=core_rknd), dimension(nlevel,sclr_dim), intent(in) :: sclrpthlp
...
! Thermodynamic level for which calculations are taking place.
            REAL(KIND=core_rknd), dimension(nlevel,hydromet_dim), intent(in) :: rtphmp
            REAL(KIND=core_rknd), dimension(nlevel,hydromet_dim), intent(in) :: wphydrometp
            REAL(KIND=core_rknd), dimension(nlevel,hydromet_dim), intent(in) :: wp2hmp
            REAL(KIND=core_rknd), dimension(nlevel,hydromet_dim), intent(in) :: thlphmp
...
! Output Variables
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wp2thlp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: cloud_frac
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wprtpthlp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wpthlp2
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: ice_supersat_frac
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: rcm
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wpthvp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wp2thvp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: rtpthvp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: thlpthvp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wprcp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wp2rcp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: rtprcp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wp4
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wprtp2
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: thlprcp
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: rcp2
            REAL(KIND=core_rknd), intent(out) ,dimension(nlevel) :: wp2rtp
! w'^4                  [m^4/s^4]
! w' r_t'               [(m kg)/(s kg)]
...
! w' r_t' th_l'         [(m kg K)/(s kg)]
            TYPE(pdf_parameter), intent(out) ,dimension(nlevel) :: pdf_params
! pdf paramters         [units vary]
            INTEGER, intent(out) :: err_code
! Are the outputs usable numbers?
! Output (passive scalar variables)
            REAL(KIND=core_rknd), intent(out), dimension(nlevel,sclr_dim) :: wpsclrprtp
            REAL(KIND=core_rknd), intent(out), dimension(nlevel,sclr_dim) :: wpsclrpthlp
            REAL(KIND=core_rknd), intent(out), dimension(nlevel,sclr_dim) :: sclrprcp
            REAL(KIND=core_rknd), intent(out), dimension(nlevel,sclr_dim) :: wp2sclrp
            REAL(KIND=core_rknd), intent(out), dimension(nlevel,sclr_dim) :: sclrpthvp
            REAL(KIND=core_rknd), intent(out), dimension(nlevel,sclr_dim) :: wpsclrp2
! Local Variables
...
! Thermodynamic quantity
            REAL(KIND=core_rknd), intent(out),dimension(nlevel) :: rc_coef
...
! Temp. flag liq./ice water loading test
            ...
            INTEGER :: level_index
!------------------------ Code Begins ----------------------------------
!print *,"nlevel",nlevel,"BB",hydromet_dim,"BB",p_in_pa,"BB", exner,"BB", thv_ds,"BB", wm,"BB", wp2,"BB", wp3,"BB", sigma_sqd_w,"BB", skw,"BB", rtm,"BB", rtp2,"BB",wprtp,"BB", thlm,"BB",thlp2,"BB", wpthlp,"BB", rtpthlp,"BB", sclrm,"BB", wpsclrp,"BB", sclrp2,"BB", sclrprtp,"BB", sclrpthlp,"BB", wphydrometp,"BB", wp2hmp,"BB", rtphmp,"BB",thlphmp

!$omp parallel

!$omp do schedule(dynamic,2)
do level_index = 1, nlevel, 1
...
! varnce_w_2, etc.) are non-zero.
    if ( wp2(level_index) <= w_tol_sqd )  then
      mixt_frac    = one_half
      w_1          = wm(level_index)
      w_2          = wm(level_index)
      varnce_w_1   = 0._core_rknd
      varnce_w_2   = 0._core_rknd
      rt_1         = rtm(level_index)
      rt_2         = rtm(level_index)
      alpha_rt     = one_half
      varnce_rt_1  = 0._core_rknd
      varnce_rt_2  = 0._core_rknd
      thl_1        = thlm(level_index)
      thl_2        = thlm(level_index)
      alpha_thl    = one_half
      ...
      if ( l_scalar_calc ) then
        do i = 1, sclr_dim, 1
          sclr1(i)        = sclrm(level_index,i)
          sclr2(i)        = sclrm(level_index,i)
...
! does the 2nd PDF component.
       if ( abs( Skw(level_index)) <= 1e-5_core_rknd ) then
          mixt_frac = one_half
       else
          mixt_frac = one_half * ( one - Skw(level_index)/ &
             sqrt( 4.0_core_rknd*( one - sigma_sqd_w(level_index) )**3 + Skw(level_index)**2 ) )
       endif
! Determine sqrt( wp2 ) here to avoid re-computing it
      sqrt_wp2 = sqrt( wp2(level_index))
...
! 1.0 standard deviation above the overall mean for w.
      w_1_n = sqrt( ( (one-mixt_frac)/mixt_frac )*(one-sigma_sqd_w(level_index)) )
...
! 3 Nov 2003
      ...
      if ( thlp2(level_index) <= thl_tol**2 ) then
        thl_1        = thlm(level_index)
        thl_2        = thlm(level_index)
...
!       thl_2_n = - (wpthlp/(sqrt( wp2 )*sqrt( thlp2 )))/w_1_n
        thl_1 = thlm(level_index) - ( wpthlp(level_index)/sqrt_wp2 )/w_2_n
        thl_2 = thlm(level_index) - ( wpthlp(level_index)/sqrt_wp2 )/w_1_n
        alpha_thl = one_half * ( one - wpthlp(level_index)*wpthlp(level_index) / &
           ((one-sigma_sqd_w(level_index))*wp2(level_index)*thlp2(level_index)) )
...
!   to generalize scalar skewnesses.  05 Nov 03
        varnce_thl_1 = ( alpha_thl / mixt_frac * thlp2(level_index) ) * width_factor_1
        varnce_thl_2 = ( alpha_thl / (one-mixt_frac) * thlp2(level_index) ) * width_factor_2
      end if ! thlp2 <= thl_tol**2 ! thlp2 <= thl_tol**2
      if ( rtp2(level_index) <= rt_tol**2 ) then
        rt_1        = rtm(level_index)
        rt_2        = rtm(level_index)
...
!       rt_2_n = -( wprtp / ( sqrt( wp2 )*sqrt( rtp2 ) ) ) / w_1_n
        rt_1 = rtm(level_index) - ( wprtp(level_index) / sqrt_wp2 ) / w_2_n
        rt_2 = rtm(level_index) - ( wprtp(level_index) / sqrt_wp2 ) / w_1_n
        alpha_rt = one_half * ( one - wprtp(level_index)*wprtp(level_index) / &
           ((one-sigma_sqd_w(level_index))*wp2(level_index)*rtp2(level_index)) )
        alpha_rt = max( min( alpha_rt, one ), zero_threshold )
! Vince Larson multiplied original expressions by width_factor_1,2
!   to generalize scalar skewnesses.  05 Nov 03
        varnce_rt_1 = ( alpha_rt / mixt_frac * rtp2(level_index)) * width_factor_1
        varnce_rt_2 = ( alpha_rt / (one-mixt_frac) * rtp2(level_index)) * width_factor_2
      end if ! rtp2 <= rt_tol**2 ! rtp2 <= rt_tol**2
! Compute pdf parameters for passive scalars
      if ( l_scalar_calc ) then
!DIR$ VECTOR ALIGNED
        do i = 1, sclr_dim
          if ( sclrp2(level_index,i) <= sclr_tol(i)**2 ) then
...
!                        * sqrt( sclrp2(i) )) )/w_1_n
            sclr1(i) = sclrm(level_index,i)  & 
                     - ( wpsclrp(level_index,i) / sqrt_wp2 ) / w_2_n
            sclr2(i) = sclrm(level_index,i)  & 
                     - ( wpsclrp(level_index,i) / sqrt_wp2 ) / w_1_n
            alpha_sclr(i) = one_half * ( one - wpsclrp(level_index,i)*wpsclrp(level_index,i) & 
                    / ((one-sigma_sqd_w(level_index))*wp2(level_index)*sclrp2(level_index,i)))
            alpha_sclr(i) = max( min( alpha_sclr(i), one ), zero_threshold )
! Vince Larson multiplied original expressions by width_factor_1,2
!  to generalize scalar skewnesses.  05 Nov 03
            varnce_sclr1(i) = ( alpha_sclr(i) / mixt_frac * sclrp2(level_index,i) ) * width_factor_1
            varnce_sclr2(i) = ( alpha_sclr(i) / (one-mixt_frac) * &
                sclrp2(level_index,i) ) * width_factor_2
...
! We include sub-plume correlation with coeff rrtthl.
      if ( varnce_rt_1*varnce_thl_1 > 0._core_rknd .and. &
        ...
        rrtthl = ( rtpthlp(level_index) - mixt_frac * ( rt_1-rtm(level_index)) * ( thl_1-thlm(level_index)) & 
                   - (one-mixt_frac) * ( rt_2-rtm(level_index)) * ( thl_2-thlm(level_index)) ) & 
      ...
      if ( l_scalar_calc ) then
        do i=1, sclr_dim
          if ( varnce_sclr1(i)*varnce_thl_1 > 0._core_rknd .and. &
            ...
            rsclrthl(i) = ( sclrpthlp(level_index,i)  & 
            - mixt_frac * ( sclr1(i)-sclrm(level_index,i) ) * ( thl_1-thlm(level_index)) & 
            - (one-mixt_frac) * ( sclr2(i)-sclrm(level_index,i) ) * ( thl_2-thlm(level_index)) ) & 
...
! Sub-plume correlation, rsclrrt, of passive scalar and total water.
          if ( varnce_sclr1(i)*varnce_rt_1 > 0._core_rknd .and. &
            ...
            rsclrrt(i) = ( sclrprtp(level_index,i) - mixt_frac * ( sclr1(i)-sclrm(level_index,i) ) * ( rt_1-rtm(level_index))&
                         - (one-mixt_frac) * ( sclr2(i)-sclrm(level_index,i) ) * ( rt_2-rtm(level_index) ) ) & 
...
! Compute higher order moments (these are interactive)
    wp2rtp(level_index)  = mixt_frac * ( (w_1-wm(level_index))**2+varnce_w_1 ) * ( rt_1-rtm(level_index)) & 
            + (one-mixt_frac) * ( (w_2-wm(level_index))**2+varnce_w_2 ) * (rt_2-rtm(level_index))
    wp2thlp(level_index) = mixt_frac * ( (w_1-wm(level_index))**2+varnce_w_1 ) * ( thl_1-thlm(level_index)) & 
            + (one-mixt_frac) * ( (w_2-wm(level_index))**2+varnce_w_2 ) * (thl_2-thlm(level_index))
! Compute higher order moments (these are non-interactive diagnostics)
    if ( iwp4 > 0 ) then
      wp4(level_index) = mixt_frac * ( 3._core_rknd*varnce_w_1**2 + &
          6._core_rknd*((w_1-wm(level_index))**2)*varnce_w_1 + (w_1-wm(level_index))**4 ) & 
          + (one-mixt_frac) * ( 3._core_rknd*varnce_w_2**2 + &
          6._core_rknd*((w_2-wm(level_index))**2)*varnce_w_2 + (w_2-wm(level_index))**4 )
    end if
    if ( iwprtp2 > 0 ) then
      wprtp2(level_index)  = mixt_frac * ( w_1-wm(level_index))*( (rt_1-rtm(level_index))**2 + varnce_rt_1 )  & 
              + (one-mixt_frac) * ( w_2-wm(level_index))*( (rt_2-rtm(level_index))**2 + varnce_rt_2)
    end if
    if ( iwpthlp2 > 0 ) then
      wpthlp2(level_index) = mixt_frac * ( w_1-wm(level_index) )*( (thl_1-thlm(level_index))**2 + varnce_thl_1 )  & 
              + (one-mixt_frac) * ( w_2-wm(level_index))*( (thl_2-thlm(level_index))**2+varnce_thl_2 )
    end if
    if ( iwprtpthlp > 0 ) then
      wprtpthlp(level_index) = mixt_frac * ( w_1-wm(level_index))*( (rt_1-rtm(level_index))*(thl_1-thlm(level_index))  & 
                + rrtthl*sqrt( varnce_rt_1*varnce_thl_1 ) ) & 
                + ( one-mixt_frac ) * ( w_2-wm(level_index))*(rt_2-rtm(level_index))*(thl_2-thlm(level_index)) & 
                + rrtthl*sqrt( varnce_rt_2*varnce_thl_2 ) 
    end if
! Scalar Addition to higher order moments
    if ( l_scalar_calc ) then
      do i=1, sclr_dim
        wp2sclrp(level_index,i)  = mixt_frac * ( (w_1-wm(level_index))**2+varnce_w_1 )*( sclr1(i)-sclrm(level_index,i) ) & 
                     + (one-mixt_frac) * ( (w_2-wm(level_index))**2+varnce_w_2 ) * ( sclr2(i)-sclrm(level_index,i) )
        wpsclrp2(level_index,i) = mixt_frac * ( w_1-wm(level_index)) * ( (sclr1(i)-sclrm(level_index,i))**2 + varnce_sclr1(i) )  & 
                    + (one-mixt_frac) * ( w_2-wm(level_index) ) * &
                    ( (sclr2(i)-sclrm(level_index,i))**2 + varnce_sclr2(i) )
        wpsclrprtp(level_index,i) = mixt_frac * ( w_1-wm(level_index)) * ( ( rt_1-rtm(level_index))*( sclr1(i)-sclrm(level_index,i) )  & 
          + rsclrrt(i)*sqrt( varnce_rt_1*varnce_sclr1(i) ) ) &
          + ( one-mixt_frac )*( w_2-wm(level_index)) *  &
            ( ( rt_2-rtm(level_index))*( sclr2(i)-sclrm(level_index,i) ) + rsclrrt(i)*sqrt( varnce_rt_2*varnce_sclr2(i) ) )
        wpsclrpthlp(level_index,i) = mixt_frac * ( w_1-wm(level_index)) * ( ( sclr1(i)-sclrm(level_index,i) )*( thl_1-thlm(level_index))  & 
          + rsclrthl(i)*sqrt( varnce_sclr1(i)*varnce_thl_1 ) ) & 
          + ( one-mixt_frac ) * ( w_2-wm(level_index)) * &
            ( ( sclr2(i)-sclrm(level_index,i) )*( thl_2-thlm(level_index)) &
...
! liq water temp (Sommeria & Deardorff 1977 (SD), eqn. 3)
    tl1  = thl_1*exner(level_index)
    tl2  = thl_2*exner(level_index)
    rsatl_1 = sat_mixrat_liq( p_in_Pa(level_index), tl1 )
    rsatl_2 = sat_mixrat_liq( p_in_Pa(level_index), tl2 ) ! h1g, 2010-06-16 end mod ! h1g, 2010-06-16 end mod
! SD's beta (eqn. 8)
...
! Larson et al. May, 2001.
    ...
    cthl_1 = ( (one + beta1 * rt_1) / ( one + beta1*rsatl_1)**2 ) & 
             * ( Cp/Lv ) * beta1 * rsatl_1 * exner(level_index)
    cthl_2 = ( (one + beta2 * rt_2) / ( one + beta2*rsatl_2 )**2 ) & 
             * ( Cp/Lv ) * beta2 * rsatl_2 * exner(level_index)
...
! We must compute chi_at_ice_sat1 and chi_at_ice_sat2
      if (tl1 <= T_freeze_K) then
        rt_at_ice_sat1 = sat_mixrat_ice( p_in_Pa(level_index), tl1 )
...
! is not defined, so we use chi_at_liq_sat
      ...
      if (tl2 <= T_freeze_K) then
        rt_at_ice_sat2 = sat_mixrat_ice( p_in_Pa(level_index), tl2 )
...
! where thv_ds is used as a reference value to approximate theta_l.
    rc_coef(level_index) = Lv / (exner(level_index)*Cp) - ep2 * thv_ds(level_index)
    wp2rxp  = zero
    wprxp   = zero
    thlprxp = zero
    rtprxp  = zero
    if ( l_liq_ice_loading_test ) then
       do hm_idx = 1, hydromet_dim, 1
          if ( l_mix_rat_hm(hm_idx) ) then
             wp2rxp  = wp2rxp + wp2hmp(level_index,hm_idx)
             wprxp   = wprxp + wphydrometp(level_index,hm_idx)
             thlprxp = thlprxp + thlphmp(level_index,hm_idx)
             rtprxp  = rtprxp + rtphmp(level_index,hm_idx)
          endif
       enddo ! hm_idx = 1, hydromet_dim, 1 ! hm_idx = 1, hydromet_dim, 1
    endif ! l_liq_ice_loading_test ! l_liq_ice_loading_test
    wp2rcp(level_index) = mixt_frac * ((w_1-wm(level_index))**2 + varnce_w_1)*rc_1 &
               + (one-mixt_frac) * ((w_2-wm(level_index))**2 + varnce_w_2)*rc_2 & 
             - wp2(level_index) * (mixt_frac*rc_1+(one-mixt_frac)*rc_2)
    wp2thvp(level_index) = wp2thlp(level_index) + ep1*thv_ds(level_index)*wp2rtp(level_index) + rc_coef(level_index)*wp2rcp(level_index) - thv_ds(level_index) * wp2rxp
    wprcp(level_index) = mixt_frac * (w_1-wm(level_index))*rc_1 + (one-mixt_frac) * (w_2-wm(level_index))*rc_2
    wpthvp(level_index) = wpthlp(level_index) + ep1*thv_ds(level_index)*wprtp(level_index) + rc_coef(level_index)*wprcp(level_index) - thv_ds(level_index) * wprxp
! Account for subplume correlation in qt-thl
    thlprcp(level_index)  = mixt_frac * ( (thl_1-thlm(level_index))*rc_1 - (cthl_1*varnce_thl_1)*cloud_frac_1 ) & 
             + (one-mixt_frac) * ( (thl_2-thlm(level_index))*rc_2 - (cthl_2*varnce_thl_2)*cloud_frac_2 ) & 
    ...
    thlpthvp(level_index) = thlp2(level_index) + ep1*thv_ds(level_index)*rtpthlp(level_index) + rc_coef(level_index)*thlprcp(level_index) - thv_ds(level_index) * thlprxp
! Account for subplume correlation in qt-thl
    rtprcp(level_index) = mixt_frac * ( (rt_1-rtm(level_index))*rc_1 + (crt_1*varnce_rt_1)*cloud_frac_1 ) & 
           + (one-mixt_frac) * ( (rt_2-rtm(level_index))*rc_2 + (crt_2*varnce_rt_2)*cloud_frac_2 ) & 
    ...
    rtpthvp(level_index)  = rtpthlp(level_index) + ep1*thv_ds(level_index)*rtp2(level_index) + rc_coef(level_index)*rtprcp(level_index) - thv_ds(level_index) * rtprxp
...
!  where the ``scalar'' in this paper is w.
    if ( l_scalar_calc ) then
      do i=1, sclr_dim
        sclrprcp(level_index,i) &
        = mixt_frac * ( ( sclr1(i)-sclrm(level_index,i) ) * rc_1 ) &
          + (one-mixt_frac) * ( ( sclr2(i)-sclrm(level_index,i) ) * rc_2 ) & 
        ...
        sclrpthvp(level_index,i) = sclrpthlp(level_index,i) + ep1*thv_ds(level_index)*sclrprtp(level_index,i) + rc_coef(level_index)*sclrprcp(level_index,i)
      end do ! i=1, sclr_dim ! i=1, sclr_dim
    end if ! l_scalar_calc ! l_scalar_calc
! Compute mean cloud fraction and cloud water
    cloud_frac(level_index) = calc_cloud_frac(cloud_frac_1, cloud_frac_2, mixt_frac)
    rcm(level_index)        = mixt_frac * rc_1         + (one-mixt_frac) * rc_2
    rcm(level_index) = max( zero_threshold, rcm(level_index))
    if (l_calc_ice_supersat_frac) then
! Compute ice cloud fraction, ice_supersat_frac
      ice_supersat_frac(level_index) = calc_cloud_frac(ice_supersat_frac1, ice_supersat_frac2, mixt_frac)
    else
! ice_supersat_frac will be garbage if computed as above
      ice_supersat_frac(level_index) = 0.0_core_rknd
...
! This is not needed for closure.  Statistical Analysis only.
      rcp2(level_index) = mixt_frac * ( chi_1*rc_1 + cloud_frac_1*stdev_chi_1**2 ) &
             + ( one-mixt_frac ) * ( chi_2*rc_2 + cloud_frac_2*stdev_chi_2**2 ) - rcm(level_index)**2
      rcp2(level_index) = max( zero_threshold, rcp2(level_index) )
! Save PDF parameters
    pdf_params(level_index)%w_1             = w_1
    pdf_params(level_index)%w_2             = w_2
    pdf_params(level_index)%varnce_w_1      = varnce_w_1
    pdf_params(level_index)%varnce_w_2      = varnce_w_2
    pdf_params(level_index)%rt_1            = rt_1
    pdf_params(level_index)%rt_2            = rt_2
    pdf_params(level_index)%varnce_rt_1     = varnce_rt_1
    pdf_params(level_index)%varnce_rt_2     = varnce_rt_2
    pdf_params(level_index)%thl_1           = thl_1
    pdf_params(level_index)%thl_2           = thl_2
    pdf_params(level_index)%varnce_thl_1    = varnce_thl_1
    pdf_params(level_index)%varnce_thl_2    = varnce_thl_2
    pdf_params(level_index)%rrtthl          = rrtthl
    pdf_params(level_index)%alpha_thl       = alpha_thl
    pdf_params(level_index)%alpha_rt        = alpha_rt
    pdf_params(level_index)%crt_1           = crt_1
    pdf_params(level_index)%crt_2           = crt_2
    pdf_params(level_index)%cthl_1          = cthl_1
    pdf_params(level_index)%cthl_2          = cthl_2
    pdf_params(level_index)%chi_1           = chi_1
    pdf_params(level_index)%chi_2           = chi_2
    pdf_params(level_index)%stdev_chi_1     = stdev_chi_1
    pdf_params(level_index)%stdev_chi_2     = stdev_chi_2
    pdf_params(level_index)%stdev_eta_1     = stdev_eta_1
    pdf_params(level_index)%stdev_eta_2     = stdev_eta_2
    pdf_params(level_index)%covar_chi_eta_1 = covar_chi_eta_1
    pdf_params(level_index)%covar_chi_eta_2 = covar_chi_eta_2
    pdf_params(level_index)%corr_chi_eta_1  = corr_chi_eta_1
    pdf_params(level_index)%corr_chi_eta_2  = corr_chi_eta_2
    pdf_params(level_index)%rsatl_1         = rsatl_1
    pdf_params(level_index)%rsatl_2         = rsatl_2
    pdf_params(level_index)%rc_1            = rc_1
    pdf_params(level_index)%rc_2            = rc_2
    pdf_params(level_index)%cloud_frac_1    = cloud_frac_1
    pdf_params(level_index)%cloud_frac_2    = cloud_frac_2
    pdf_params(level_index)%mixt_frac       = mixt_frac

    if ( clubb_at_least_debug_level( 2 ) ) then
      call pdf_closure_check & 
           ( wp4(level_index), wprtp2(level_index), wp2rtp(level_index), wpthlp2(level_index), & 
             wp2thlp(level_index), cloud_frac(level_index), rcm(level_index), wpthvp(level_index), wp2thvp(level_index), & 
             rtpthvp(level_index), thlpthvp(level_index), wprcp(level_index), wp2rcp(level_index), & 
             rtprcp(level_index), thlprcp(level_index), rcp2(level_index), wprtpthlp(level_index), & 
             crt_1, crt_2, cthl_1, cthl_2, pdf_params(level_index), &
             sclrpthvp(level_index,:), sclrprcp(level_index,:), wpsclrp2(level_index,:), & 
             wpsclrprtp(level_index,:), wpsclrpthlp(level_index,:),wp2sclrp(level_index,:), &
             err_code )
! Error Reporting
! Joshua Fasching February 2008
      if ( fatal_error( err_code ) ) then
        ...
        write(fstderr,*) "p_in_Pa = ", p_in_Pa(level_index)
        write(fstderr,*) "exner = ", exner(level_index)
        write(fstderr,*) "thv_ds = ", thv_ds(level_index)
        write(fstderr,*) "wm = ", wm(level_index)
        write(fstderr,*) "wp2 = ", wp2(level_index)
        write(fstderr,*) "wp3 = ", wp3(level_index)
        write(fstderr,*) "sigma_sqd_w = ", sigma_sqd_w(level_index)
        write(fstderr,*) "rtm = ", rtm(level_index)
        write(fstderr,*) "rtp2 = ", rtp2(level_index)
        write(fstderr,*) "wprtp = ", wprtp(level_index)
        write(fstderr,*) "thlm = ", thlm(level_index)
        write(fstderr,*) "thlp2 = ", thlp2(level_index)
        write(fstderr,*) "wpthlp = ", wpthlp(level_index)
        write(fstderr,*) "rtpthlp = ", rtpthlp(level_index)
        if ( sclr_dim > 0 ) then
          write(fstderr,*) "sclrm = ", sclrm(level_index,:)
          write(fstderr,*) "wpsclrp = ", wpsclrp(level_index,:)
          write(fstderr,*) "sclrp2 = ", sclrp2(level_index,:)
          write(fstderr,*) "sclrprtp = ", sclrprtp(level_index,:)
          write(fstderr,*) "sclrpthlp = ", sclrpthlp(level_index,:)
        end if
        !write(fstderr,*) "level = ", level
        write(fstderr,*) "Intent(out)"
        write(fstderr,*) "wp4 = ", wp4(level_index)
        write(fstderr,*) "wprtp2 = ", wprtp2(level_index)
        write(fstderr,*) "wp2rtp = ", wp2rtp(level_index)
        write(fstderr,*) "wpthlp2 = ", wpthlp2(level_index)
        write(fstderr,*) "cloud_frac = ", cloud_frac(level_index)
        write(fstderr,*) "ice_supersat_frac = ", ice_supersat_frac(level_index)
        write(fstderr,*) "rcm = ", rcm(level_index)
        write(fstderr,*) "wpthvp = ", wpthvp(level_index)
        write(fstderr,*) "wp2thvp = ", wp2thvp(level_index)
        write(fstderr,*) "rtpthvp = ", rtpthvp(level_index)
        write(fstderr,*) "thlpthvp = ", thlpthvp(level_index)
        write(fstderr,*) "wprcp = ", wprcp(level_index)
        write(fstderr,*) "wp2rcp = ", wp2rcp(level_index)
        write(fstderr,*) "rtprcp = ", rtprcp(level_index)
        write(fstderr,*) "thlprcp = ", thlprcp(level_index)
        write(fstderr,*) "rcp2 = ", rcp2(level_index)
        write(fstderr,*) "wprtpthlp = ", wprtpthlp(level_index)
        write(fstderr,*) "pdf_params%w_1 = ", pdf_params(level_index)%w_1
        write(fstderr,*) "pdf_params%w_2 = ", pdf_params(level_index)%w_2
        write(fstderr,*) "pdf_params%varnce_w_1 = ", pdf_params(level_index)%varnce_w_1
        write(fstderr,*) "pdf_params%varnce_w_2 = ", pdf_params(level_index)%varnce_w_2
        write(fstderr,*) "pdf_params%rt_1 = ", pdf_params(level_index)%rt_1
        write(fstderr,*) "pdf_params%rt_2 = ", pdf_params(level_index)%rt_2
        write(fstderr,*) "pdf_params%varnce_rt_1 = ", pdf_params(level_index)%varnce_rt_1
        write(fstderr,*) "pdf_params%varnce_rt_2 = ", pdf_params(level_index)%varnce_rt_2
        write(fstderr,*) "pdf_params%thl_1 = ", pdf_params(level_index)%thl_1
        write(fstderr,*) "pdf_params%thl_2 = ", pdf_params(level_index)%thl_2
        write(fstderr,*) "pdf_params%varnce_thl_1 = ", pdf_params(level_index)%varnce_thl_1
        write(fstderr,*) "pdf_params%varnce_thl_2 = ", pdf_params(level_index)%varnce_thl_2
        write(fstderr,*) "pdf_params%rrtthl = ", pdf_params(level_index)%rrtthl
        write(fstderr,*) "pdf_params%alpha_thl = ", pdf_params(level_index)%alpha_thl
        write(fstderr,*) "pdf_params%alpha_rt = ", pdf_params(level_index)%alpha_rt
        write(fstderr,*) "pdf_params%crt_1 = ", pdf_params(level_index)%crt_1
        write(fstderr,*) "pdf_params%crt_2 = ", pdf_params(level_index)%crt_2
        write(fstderr,*) "pdf_params%cthl_1 = ", pdf_params(level_index)%cthl_1
        write(fstderr,*) "pdf_params%cthl_2 = ", pdf_params(level_index)%cthl_2
        write(fstderr,*) "pdf_params%chi_1 = ", pdf_params(level_index)%chi_1
        write(fstderr,*) "pdf_params%chi_2 = ", pdf_params(level_index)%chi_2
        write(fstderr,*) "pdf_params%stdev_chi_1 = ", pdf_params(level_index)%stdev_chi_1
        write(fstderr,*) "pdf_params%stdev_chi_2 = ", pdf_params(level_index)%stdev_chi_2
        write(fstderr,*) "pdf_params%stdev_eta_1 = ", pdf_params(level_index)%stdev_eta_1
        write(fstderr,*) "pdf_params%stdev_eta_2 = ", pdf_params(level_index)%stdev_eta_2
        write(fstderr,*) "pdf_params%covar_chi_eta_1 = ", pdf_params(level_index)%covar_chi_eta_1
        write(fstderr,*) "pdf_params%covar_chi_eta_2 = ", pdf_params(level_index)%covar_chi_eta_2
        write(fstderr,*) "pdf_params%corr_chi_eta_1 = ", pdf_params(level_index)%corr_chi_eta_1
        write(fstderr,*) "pdf_params%corr_chi_eta_2 = ", pdf_params(level_index)%corr_chi_eta_2
        write(fstderr,*) "pdf_params%rsatl_1 = ", pdf_params(level_index)%rsatl_1
        write(fstderr,*) "pdf_params%rsatl_2 = ", pdf_params(level_index)%rsatl_2
        write(fstderr,*) "pdf_params%rc_1 = ", pdf_params(level_index)%rc_1
        write(fstderr,*) "pdf_params%rc_2 = ", pdf_params(level_index)%rc_2
        write(fstderr,*) "pdf_params%cloud_frac_1 = ", pdf_params(level_index)%cloud_frac_1
        write(fstderr,*) "pdf_params%cloud_frac_2 = ", pdf_params(level_index)%cloud_frac_2
        write(fstderr,*) "pdf_params%mixt_frac = ", pdf_params(level_index)%mixt_frac
        if ( sclr_dim > 0 )then
...
!$omp end parallel
    ...
    return
        END SUBROUTINE pdf_closure_test
!=============================================================================
...
!-----------------------------------------------------------------------
    END MODULE pdf_closure_module_test
