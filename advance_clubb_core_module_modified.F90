...
! Procedure
            ...
            USE pdf_closure_module_test, ONLY:pdf_closure_test
! Procedure
...
! Interpolate hydrometeor mixed moments to momentum levels.
   ...
   ! do k = 1, gr%nz, 1
      call pdf_closure_test & 
        (gr%nz,hydromet_dim, p_in_Pa, exner, thv_ds_zt, wm_zt, & ! intent(in)
          wp2_zt, wp3, sigma_sqd_w_zt,                       & ! intent(in)
          Skw_zt, rtm, rtp2_zt,                              & ! intent(in)
...
! Call pdf_closure to output the variables which belong on the momentum grid.
      !do k = 1, gr%nz, 1
!        call pdf_closure_test & 
 !         ( gr%nz,hydromet_dim, p_in_Pa_zm, exner_zm, thv_ds_zm, wm_zm, & ! intent(in)
 !           wp2, wp3_zm, sigma_sqd_w,                                & ! intent(in)
 !           Skw_zm, rtm_zm, rtp2,                                    & ! intent(in)
...
!Also added rain for completeness. storer-3/4/14
  ...
  !        (gr%nz, hydromet_dim, p_in_Pa, exner, thv_ds_zt, wm_zt,           & ! intent(in)
  !          wp2_zt, wp3, sigma_sqd_w_zt,                                 & ! intent(in)
  !          Skw_zt, rtm_frz, rtp2_zt,                                    & ! intent(in)
...
!        do k=1, gr%nz, 1
    ...
    !        ( gr%nz,hydromet_dim, p_in_Pa_zm, exner_zm, thv_ds_zm, wm_zm, & ! intent(in)
    !          wp2, wp3_zm, sigma_sqd_w,                                & ! intent(in)
    !          Skw_zm, rtm_zm_frz, rtp2,                                & ! intent(in)
